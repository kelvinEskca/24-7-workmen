...
const CategoryList = () => {
    ...
    const handleDelete = async (i) =>{
        const id = i._id;
        try{
            const res = await axios.post(`http://localhost:5000/api/category/delete/${id}`,{
                id:id
            },{ headers:{token:token} });
            if(res.status === 200){
                        ...
                        <div className="boxes">
                            ...
                            <div className="tabs-wrapper">
                                <div className="tabs">
                                    ...
                                    <Link to="/categoryList"><div className="tab active"><h3 className="heading">View Categories</h3></div></Link>
                                    <Link to="/blogList"><div className="tab"><h3 className="heading">View Blogs</h3></div></Link>
                                    <Link to="/category"><button>Add Category</button></Link>
                                </div>
                            ...
                            <div className="details-wrapper list cat">
                                <div className="table-header">
                                    ...
                                    <div className="table"><h3 className="heading">Catgeory Name</h3></div>
                                    <div className="table"><h3 className="heading">Category Image</h3></div>
                                    <div className="table"><h3 className="heading">Number of Users</h3></div>
                                ...
                                {category.map((item,i)=>{
                                    return (
                                        <div className="table-body high-table" key={i}>
                                            ...
                                            <div className="table"><p className="paragraph">{item.categoryname}</p></div>
                                            <div className="table"><img src={`../assets/${item.image[0].originalname}`} alt={item.title} style={{width:"100px",height:"6rem"}}/></div>
                                            <div className="table"><p className="paragraph">{10}</p></div>
                                            <div className="table action"><Link to={`/categoryview/${item._id}`}><button>View</button></Link><Link to="/categoryList"><button onClick={()=>handleDelete(item)}>Delete</button></Link></div>
                                        </div>
                                    );
                                })}
                            </div>
